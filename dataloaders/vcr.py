...
class VCR(Dataset):
    ...
    def _get_dets_to_use(self, item):
        ...
        if self.only_use_relevant_dets:
            ...
            if self.expand2obj36:
                if np.sum(dets2use) >= 36:  # 本身提到的就多于36个Object
                    pass
                else:
    ...
    def __getitem__(self, index):
        ...
        boxes_box = np.array(metadata['boxes'])[dets2use, :-1]
        ...
        boxes_feat[:, 0] = (boxes_box[:, 0] + boxes_box[:, 2]) / 2
        boxes_feat[:, 1] = (boxes_box[:, 1] + boxes_box[:, 3]) / 2
        boxes_feat[:, 2] = boxes_box[:, 2] - boxes_box[:, 0]
        boxes_feat[:, 3] = boxes_box[:, 3] - boxes_box[:, 1]
        boxes_feat[:, 4] = boxes_feat[:, 2] * boxes_feat[:, 3]

        max_r = np.max(boxes_feat, axis=0)  # 所有维度最大的值
        min_r = np.min(boxes_feat, axis=0)
        r_range = max_r - min_r
        boxes_feat = (boxes_feat - min_r) / r_range

        # boxes_feat[0]: the whole img

        instance_dict['boxes_feat'] = ArrayField(boxes_feat, padding_value=0)
