...
def Select_obj_new_topn(v_grad, box_mask, topn, batch_size, obj_num):
    topn = min(topn, obj_num)
    v_grad_ind = v_grad.sort(1, descending=True)[1]  # index sort result
    v_mask_top_pos = torch.zeros(batch_size, obj_num).cuda().long()
    v_mask_top_neg = torch.zeros(batch_size, obj_num).cuda().long()
    top_pos = torch.zeros(batch_size, topn).cuda().long()
    top_neg = torch.zeros(batch_size, topn).cuda().long()

    for j in range(batch_size):
        top_pos[j] = v_grad_ind[j][:topn]

        if 0 in top_pos[j]: # do not contain the whole img
            image_idx = list(top_pos[j]).index(0)
            top_pos[j][image_idx] = v_grad_ind[j][topn] # not include img, pick the next

        badest = box_mask[j].sum() - 1
        if badest < topn:
            # top_neg[j][:badest] = v_grad_ind[j][badest - 3:badest]
            top_neg[j][:badest] = v_grad_ind[j][:badest]
            if 0 in top_neg[j]:
                image_idx = list(top_neg[j]).index(0)
                top_neg[j][image_idx] = v_grad_ind[j][badest-3-1] # pick the previous
        else:
    ...
    #v_mask_neg = v_mask_neg.transpose(1,0)
